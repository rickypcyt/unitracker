        ...
        <div className="space-y-4 p-6">
          ...
          <div className="p-6">
            {months.length === 0 ? <div className="text-center py-8">
              ...
              </div> : selectedMonth ? <div className="space-y-4">
                <div className="space-y-2">
                  {(groupedByMonth[selectedMonth] || []).slice().sort((a, b) => {
                ...
                handleEditTask(task);
                      ...
                      e.stopPropagation();
                      handleEditTask(task);
                    }} className="p-1.5 text-[var(--text-secondary)] hover:bg-[var(--bg-primary)] rounded-md transition-colors" title="Edit task">
                                    <Edit2 size={16} />
                                  </button>
                                  <button onClick={e => {
                      e.stopPropagation();
                      handleDeleteTask(task);
                    }} className="p-1.5 text-red-500 hover:bg-red-500/10 rounded-md transition-colors" title="Delete task">
                                    <Trash2 size={16} />
                                  </button>
                                </div>
                              </div>
                              ...
                              <div className="flex items-center gap-2 text-sm text-[var(--text-secondary)] mt-3">
                                <div className="flex items-center">
                                  ...
                                  <span>Completed {task.completed_at ? formatDistanceToNow(new Date(task.completed_at), {
                        addSuffix: true
                      }) : 'recently'}</span>
                                </div>
                              </div>
                              
                              {(task as any).duration && <div className="mt-2 text-sm text-[var(--text-secondary)]">
                                  <span>Duration: {(task as any).duration} min</span>
                                </div>}
                              
                              {task.description && <p className="mt-2 text-sm text-[var(--text-secondary)] line-clamp-2">
                                  {task.description}
                                </p>}
                            </div>)}
                </div>
