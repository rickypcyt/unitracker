...
const StartSessionModal = ({
  ...
  const assignmentSuggestions = useMemo(() => [...new Set(tasks.map(task => (task.assignment ?? "").trim()).filter(value => value.length > 0))].sort((a, b) => a.localeCompare(b)), [tasks]);
  ...
  const groupedTasks = useMemo(() => {
    ...
    return Array.from(groups.entries()).sort(([a], [b]) => a === "No Assignment" ? 1 : b === "No Assignment" ? -1 : a.localeCompare(b));
  }, [tasks]);
  const fetchSessionTasks = useCallback(async () => {
    try {
      ...
      const {
        data: tasks,
        error
      } = await supabase.from("tasks").select("*").eq("user_id", data.user.id).eq("completed", false).order("created_at", {
        ascending: false
      });
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      if (selectedTasks.length > 0) {
        ...
        } = await supabase.from("session_tasks").insert(selectedTasks.map(taskId => ({
  ...
  return <BaseModal isOpen={isOpen} onClose={onClose} title="Start Session" className="w-full px-4 sm:px-5" maxWidth="max-w-[60rem]" fullWidthOnMd={false}>
      <div className="space-y-6 w-full max-w-[56rem] mx-auto">
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)] xl:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <AutocompleteInput id="assignment" value={assignment} onChange={setAssignment} placeholder="e.g., Math" suggestions={assignmentSuggestions} />
                  </div>
                </div>
                ...
                <div className="bg-[var(--bg-primary)] rounded-lg ">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <button type="button" onClick={() => setSyncPomo(v => !v)} className={`flex items-center justify-between w-full gap-3 px-3 py-2 rounded-lg border-2 transition-all duration-200 ${syncPomo ? "border-[var(--accent-primary)] bg-[var(--accent-primary)]/20 text-[var(--accent-primary)]" : "border-[var(--border-primary)] bg-[var(--bg-secondary)] hover:bg-[var(--bg-hover)] text-[var(--text-secondary)]"}`}>
                      <span className="font-medium text-sm">Start Pomodoro Timer</span>
                      {syncPomo ? <Check size={18} className="text-[var(--accent-primary)]" /> : <Square size={18} />}
                    </button>
                    <button type="button" onClick={() => setSyncCountdown(v => !v)} className={`flex items-center justify-between w-full gap-3 px-3 py-2 rounded-lg border-2 transition-all duration-200 ${syncCountdown ? "border-[var(--accent-primary)] bg-[var(--accent-primary)]/20 text-[var(--accent-primary)]" : "border-[var(--border-primary)] bg-[var(--bg-secondary)] hover:bg-[var(--bg-hover)] text-[var(--text-secondary)]"}`}>
                      <span className="font-medium text-sm">Start Countdown Timer</span>
                      {syncCountdown ? <Check size={18} className="text-[var(--accent-primary)]" /> : <Square size={18} />}
                    </button>
                  </div>
                </div>
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-[var(--bg-secondary)] rounded-xl p-4">
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                    {tasks.length === 0 ? <div className="text-center py-8 text-[var(--text-secondary)]">
                        <List size={48} className="mx-auto mb-2 opacity-50" />
                        <p className="text-sm">No tasks available</p>
                        <button onClick={() => setShowTaskForm(true)} className="mt-2 text-sm text-[var(--accent-primary)] hover:text-[var(--accent-primary)]/80">
                          Create your first task
                        </button>
                      </div> : <div className="max-h-56 overflow-y-auto pr-1 [scrollbar-color:var(--accent-primary)_transparent] space-y-3">
                        {groupedTasks.map(([assignmentLabel, grouped]) => <div key={assignmentLabel} className="space-y-3">
                        ...
                        return <div key={task.id} onClick={() => {
                          setSelectedTasks(prev => {
                        ...
                        }} className={`group relative flex w-full cursor-pointer flex-col gap-3 rounded-xl border-2 p-4 transition-all duration-200 ${isSelected ? "border-[var(--accent-primary)] bg-[var(--accent-primary)]/10 shadow-[0_10px_30px_-18px_rgba(34,197,94,0.6)]" : "border-[var(--border-primary)] bg-[var(--bg-primary)] hover:border-[var(--accent-primary)]/70 hover:bg-[var(--bg-secondary)]"}`}>
                                    <div className="flex items-center gap-3">
                                      <div className="flex-1 min-w-[200px]">
                                        <div className="text-sm font-semibold text-[var(--text-primary)]">
                                          {task.title}
                                        </div>
                                        {task.assignment && <div className="mt-1 text-xs font-medium uppercase tracking-wide text-[var(--text-tertiary)]">
                                            {task.assignment}
                                          </div>}
                                      </div>
                                      <div className={`ml-auto flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-lg border-2 transition-colors duration-200 ${isSelected ? "border-[var(--accent-primary)] text-[var(--accent-primary)] bg-transparent" : "border-[var(--border-primary)] bg-[var(--bg-secondary)] text-[var(--text-secondary)] group-hover:border-[var(--accent-primary)]"}`}>
                                        {isSelected && <Check size={14} />}
                                      </div>
                                    </div>
                                    {task.description && (task.description.includes('<') ? <div className="text-[var(--text-secondary)] text-xs leading-relaxed break-words prose prose-invert max-w-none prose-p:my-1 prose-ul:my-1 prose-ol:my-1 prose-li:my-0 prose-li:whitespace-pre-wrap" dangerouslySetInnerHTML={{
        ...
        <div className="bg-[var(--bg-secondary)] rounded-xl p-4">
          ...
          <div className="flex gap-3">
            <button type="button" onClick={onClose} className="flex-1 px-8 py-3 bg-[var(--bg-primary)] border border-[var(--border-primary)] rounded-lg font-medium text-[var(--text-primary)] hover:bg-[var(--bg-hover)] transition-all duration-200" aria-label="Cancel and close modal">
              Cancel
            </button>
            <button type="button" onClick={handleStart} disabled={isSubmitting || !sessionTitle.trim()} className={`flex-1 px-8 py-3 rounded-lg font-medium transition-all duration-200 flex items-center justify-center gap-2 ${isSubmitting || !sessionTitle.trim() ? "bg-[var(--bg-primary)] border border-[var(--border-primary)] text-[var(--text-secondary)] cursor-not-allowed" : "text-[var(--accent-primary)] border-2 border-[var(--accent-primary)] bg-transparent hover:bg-[var(--accent-primary)]/10"}`} aria-label={isSubmitting ? "Starting session" : "Start session"}>
              {isSubmitting ? <>
        ...
        {showTaskForm && <TaskForm onClose={() => setShowTaskForm(false)} onTaskCreated={newTaskId => {
