...
const AddFriendModal = ({ isOpen, onClose, onSendRequest, requests = [], onAccept, onReject, hasRequests }) => {
    ...
    <BaseModal isOpen={isOpen} onClose={onClose} title="Add Friend" maxWidth="max-w-md">
      <div className="flex flex-col items-center gap-4 py-4">
        ...
        <div className="flex items-center justify-center gap-2 mb-2">
          ...
          <button
            className={`px-4 py-1 font-semibold ${tab === 'requests' ? 'text-[var(--accent-primary)]' : 'text-[var(--text-secondary)]'} focus:outline-none relative`}
            onClick={() => setTab('requests')}
          >
            Requests
            {hasRequests && (
              <span className="absolute -top-2 -right-3 w-3 h-3 rounded-full bg-[var(--accent-primary)]"></span>
            )}
          </button>
        </div>
        {/* Content */}
        ...
        {tab === 'requests' && (
          <div className="w-full flex flex-col gap-2">
            {requests.length === 0 ? (
              <div className="text-[var(--text-secondary)] text-center">No requests</div>
            ) : (
              requests.map((req, i) => (
