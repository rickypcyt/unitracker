...
const FinishSessionModal: React.FC<FinishSessionModalProps> = ({
  ...
  return <div className="fixed inset-0 bg-white/60 dark:bg-black/70 flex items-center justify-center z-[10001] backdrop-blur-sm p-4">
      <div className="bg-[var(--bg-primary)] rounded-lg p-6 w-full max-w-3xl border-2 border-[var(--border-primary)] shadow-xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="mb-6 p-4 bg-[var(--bg-secondary)] rounded-lg border-2 border-[var(--border-primary)]">
          ...
          {isLoading ? <div className="flex items-center justify-center py-4">
            ...
            </div> : error ? <div className="flex items-center gap-2 text-red-500 py-2">
              <AlertCircle size={16} />
              <span className="text-sm">{error}</span>
            </div> : sessionStats ? <div className="grid grid-cols-3 gap-4">
          ...
          {activeTasks.length === 0 ? <div className="text-center text-gray-400 py-6 text-lg font-medium bg-[var(--bg-secondary)] rounded-lg border-2 border-[var(--border-primary)]">
              ...
              if (e.target.checked) {
                ...
                setSelectedTasks(prev => prev.filter(id => id !== task.id));
                  ...
                  <label htmlFor={`task-${task.id}`} className="flex-1 text-[var(--text-primary)] cursor-pointer">
                    {task.title}
                  </label>
                  <button onClick={() => handleRemoveTaskFromFinished(task)} className="text-red-500 hover:text-red-400 transition-colors" title="Remove from finished tasks">
                    <X size={16} />
                  </button>
      ...
      {/* Task Form Modal */}
      {showTaskForm && <TaskForm onClose={handleTaskFormClose} onTaskCreated={(newTaskId: string) => {
      fetchSessionTasks();
