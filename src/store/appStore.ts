...
export const useAppStore = create<AppState>()(persist((set, get) => ({
  ...
  setPomodoroState: state => set({
    pomodoroState: state
  }),
  updatePomodoroState: updates => set(state => ({
    pomodoroState: {
      ...state.pomodoroState,
      ...updates
    }
  ...
  setPomodoroModes: modes => set({
    pomodoroModes: modes
  }),
  updatePomodoroMode: (index, mode) => set(state => {
    const newModes = [...state.pomodoroModes];
    newModes[index] = mode;
    return {
      pomodoroModes: newModes
    };
  }),
  // Pomodoro Counts
  pomodorosThisSession: 0,
  incrementPomodorosThisSession: () => set(state => ({
    pomodorosThisSession: state.pomodorosThisSession + 1
  })),
  resetPomodorosThisSession: () => set({
    pomodorosThisSession: 0
  }),
  pomodorosTodayLocal: 0,
  incrementPomodorosTodayLocal: () => set(state => ({
    pomodorosTodayLocal: state.pomodorosTodayLocal + 1
  })),
  resetPomodorosTodayLocal: () => set({
    pomodorosTodayLocal: 0
  }),
  // ⏰ Countdown State
  countdownState: DEFAULT_COUNTDOWN_STATE,
  setCountdownState: state => set({
    countdownState: state
  }),
  updateCountdownState: updates => set(state => ({
    countdownState: {
  ...
  setCountdownBaseline: baseline => set({
    countdownBaseline: baseline
  }),
  updateCountdownBaseline: updates => set(state => ({
    countdownBaseline: {
      ...state.countdownBaseline,
      ...updates
    }
  })),
  // Countdown Settings
  countdownAlarmEnabled: true,
  toggleCountdownAlarm: () => set(state => ({
    countdownAlarmEnabled: !state.countdownAlarmEnabled
  })),
  // 📚 Study Timer State
  studyState: DEFAULT_STUDY_STATE,
  setStudyState: state => set({
    studyState: state
  }),
  updateStudyState: updates => set(state => ({
    studyState: {
      ...state.studyState,
      ...updates
    }
  ...
  setStudyTimerStartedAt: timestamp => set({
    studyTimerStartedAt: timestamp
  }),
  // Study Stats
  sessionsTodayCount: 0,
  incrementSessionsTodayCount: () => set(state => ({
    sessionsTodayCount: state.sessionsTodayCount + 1
  })),
  resetSessionsTodayCount: () => set({
    sessionsTodayCount: 0
  }),
  // 🔄 Sync Settings
  syncSettings: DEFAULT_SYNC_SETTINGS,
  setSyncSettings: settings => set({
  ...
  clearSessionSyncSettings: (sessionId: string) => set(state => {
    ...
    return {
      sessionSyncSettings: newSettings
    };
  }),
  // 📌 Pinned Columns State
  pinnedColumns: DEFAULT_PINNED_COLUMNS_STATE,
  setPinnedColumns: (pinnedColumns: PinnedColumnsState) => set({
    pinnedColumns
  }),
  togglePin: (workspaceId: string, assignment: string) => set(state => {
    const currentPins: PinnedColumnsState = {
      ...state.pinnedColumns
    };
    ...
    return {
      pinnedColumns: currentPins
    };
  }),
  isPinned: (workspaceId: string, assignment: string) => {
    return !!get().pinnedColumns[workspaceId]?.[assignment];
  },
  // Pomodoro Settings
  pomodoroSettings: DEFAULT_POMODORO_SETTINGS,
  setPomodoroSettings: settings => set({
  ...
  updateTask: (id, updates) => set(state => ({
    tasks: {
      ...state.tasks,
      tasks: state.tasks.tasks.map(task => task.id === id ? {
        ...task,
        ...updates
      } : task)
    }
  ...
  addLap: lap => set(state => ({
    laps: {
      ...state.laps,
      laps: [...state.laps.laps, lap]
    }
  })),
  updateLap: (id, updates) => set(state => ({
    laps: {
      ...state.laps,
      laps: state.laps.laps.map(lap => lap.id === id ? {
        ...lap,
        ...updates
      } : lap)
    }
  ...
  setPomodoroTimerState: state => set({
    pomodoroTimerState: state
  }),
  studyTimerState: 'stopped',
  setStudyTimerState: state => set({
    studyTimerState: state
  }),
  countdownTimerState: 'stopped',
  setCountdownTimerState: state => set({
  ...
  setSyncCountdownWithTimer: sync => set(prevState => ({
    syncSettings: {
      ...prevState.syncSettings,
      syncCountdownWithTimer: sync
    }
  })),
  setSyncPomodoroWithTimer: sync => set(prevState => ({
    syncSettings: {
      ...prevState.syncSettings,
      syncPomodoroWithTimer: sync
    }
  })),
  ...
  addTaskSuccess: task => set(prevState => ({
    tasks: {
      ...prevState.tasks,
      tasks: [...prevState.tasks.tasks, task]
    }
  })),
  updateTaskSuccess: task => {
    set(prevState => ({
      tasks: {
        ...prevState.tasks,
        tasks: prevState.tasks.tasks.map(t => t.id === task.id ? {
          ...t,
          ...task
        } : t),
        isCached: false,
  ...
  toggleTaskStatus: (id, completed) => set(prevState => ({
    tasks: {
      ...prevState.tasks,
      tasks: prevState.tasks.tasks.map(task => task.id === id ? {
        ...task,
        completed,
  ...
  onRehydrateStorage: () => state => {
    ...
    if (state) {
      ...
      const hasOldModes = currentModes.some(mode => mode.label === 'Quick Sprints' || mode.label === 'Student');
...
export const useSessionSyncSettings = (sessionId: string | null) => useAppStore(state => sessionId ? state.sessionSyncSettings[sessionId] || null : null);
