...
import { useStudyState } from '@/store/appStore';
...
const FloatingFooter: React.FC<FloatingFooterProps> = ({
  ...
  const handleQuickAddTask = () => {
    navigateTo('tasks');
    setTimeout(() => {
      window.dispatchEvent(new CustomEvent('quickAddTask'));
    }, 300);
  };
  ...
  return (
    <>
      <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 z-[9999]">
        <div className="flex items-center gap-2 px-3 py-2 rounded-full border border-[var(--border-primary)]/70 bg-[var(--bg-primary)]/70 backdrop-blur-[12px] shadow-[0_12px_30px_rgba(15,23,42,0.35)] text-[var(--text-secondary)]">
          ...
          <a
            ...
            className="flex items-center justify-center w-7 h-7 rounded-full hover:bg-[var(--bg-secondary)] hover:text-[var(--text-primary)] transition-colors"
            aria-label="Discord"
          >
            <svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" aria-hidden="true">
              <title>Discord</title>
          ...
          {showSessionTimer && (
            <>
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-2 py-0.5 rounded-full hover:bg-[var(--bg-secondary)] hover:text-[var(--text-primary)] transition-colors"
                aria-label="Active study session"
                title="Go to study session"
              >
                <span className={`w-2 h-2 rounded-full ${studyState.isRunning ? 'bg-emerald-500 animate-pulse' : 'bg-[var(--text-secondary)]'}`} />
          ...
          >
            {currentTheme === 'dark' ? (
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="text-amber-400"><circle cx="12" cy="12" r="4"></circle><path d="M12 2v2"></path><path d="M12 20v2"></path><path d="m4.93 4.93 1.41 1.41"></path><path d="m17.66 17.66 1.41 1.41"></path><path d="M2 12h2"></path><path d="M20 12h2"></path><path d="m6.34 17.66-1.41 1.41"></path><path d="m19.07 4.93-1.41 1.41"></path></svg>
            ) : (
          ...
          <button
            ...
            className="flex items-center justify-center w-7 h-7 rounded-full hover:bg-[var(--bg-secondary)] hover:text-[var(--accent-primary)] transition-colors"
